...
const Pnameon = localStorage.getItem("playername");
...
const task2 = Number(localStorage.getItem("task2"));
...
function start1() {
 PN1 = PM[0].name;
 PN2 = BOSS.name;
 localStorage.setItem("BOSSname",PN2);
 alert(PN2 + "が現れた！");
 if (BOSS.allup === true) {
  AR2 += BOSS.upN;
  BR2 += BOSS.upN;
 ...
 Bskiller = skill[25];
 if (BOSS.semiMAX === true || BOSS.MAX === true || BOSS.name === "ネクロズマ") {
  PK2.style.width = 400;
  PK2.style.top = 90;
  PK2.style.right = 150;
 };
 ...
 dmgN = BOSS.MHP / 100000;
 if (dmgN < 0.005) { // ボスのHPが500未満
  dmgmode = 1;
 } else {
  if (dmgN < 0.01) { // ボスのHPが1000未満
   dmgmode = 2;
  } else {
   if (dmgN < 0.05) { // ボスのHPが5000未満
    dmgmode = 5;
   } else {
    if (dmgN < 0.1) { // ボスのHPが10000(一万)未満
     dmgmode = 20;
    } else {
     if (dmgN < 0.5) { // ボスのHPが50000(五万)未満
      dmgmode = 50
     } else {
      if (dmgN < 1) { // ボスのHPが100000(十万)未満
       dmgmode = 200;
      } else {
       dmgmode = 500  // ボスのHPが100000(十万)以上
      };
     };
    };
   };
  ...
  if (F2 === "いかく") {
   ...
   if ((F2 === "エレキメイカー" && field !== "elec") || (F2 === "サイコメイカー" && field !== "phycho") || (F2 === "ミストメイカー" && field !== "mist")) {
    ...
    if (F2 === "すなおこし" && weather !== "sandstrom") {
     ...
     if (F2 === "おわりのだいち" || F2 === "はじまりのうみ" || F2 === "デルタストリーム") {
      weather2 = true;
      startdash2 = true;
      Fdp21.style.visibility = "visible";
      Fdp22.style.visibility = "visible";
