function test1() {
 turn += 1
 score += Math.floor(1000 * Math.sqrt(turn));
 if (MGstart === true) {
  MGstart = false;
  MGed = true;
  PM[ID].mega = true;
 ...
 if (MAXstart === true) {
  ...
  futurehp1 = (PM[ID].HP * 2 / dmgmode) * dmgmode;
  MHPdp.innerHTML = PM[ID].MHP;
...
function total() {
 if ((F1 === "いたずらごころ" && skiller.type === "change") || skiller.snowdown === true || skiller.name === "トリックルーム" || skiller.shield === true || (PM[ID].MAX === true && skiller.type === "change") || skiller.name === "しんそく" || Bskiller.name === "しんそく" || skiller.firster === true || (skiller.skyjump === true && gravities === true)) {
  if (F1 === "いたずらごころ") {
   firster1 = true;
   pt1 += 1
  };
...
function grow2() {
 if (BOSS.HP < futurehp2) {
  ...
  if (hpbars + 1 !== Math.ceil(BOSS.HP / BOSS.MHP * 100)) {
   hpbars += 1;
   hpbar[hpbars].style.color = "#2DC12B";
   hpbar[hpbars].style.backgroundColor = "#2DC12B";
  };
  setTimeout(grow2,15);
 } else {
  if (PN2 === "ジガルデ") {
 ...
 if (fishes2 === true) {
  ...
  if (Fdown2 === true) {
   ...
   if (weather2 === true) {
    ...
    if (F2 === "デルタストリーム") {
     weather = "aircraftturbulence";
     Fdp21.style.visibility = "hidden";
     Fdp22.style.visibility = "hidden";
     alert("乱気流がひこうポケモンを守る！");
    };
    if (startdash2 === true) {
     setTimeout(start6,100);
    } else {
