function rate() {
 ...
 if (first === true) {
  if (firsted === 1) {
   minus = Math.floor(minus / Bresist);
   if (F2 === "フィルター" && dmg > 2) {
    minus = Math.floor(minus / 4 * 3);
   };
   minus = Math.ceil(minus / dmgmode) * dmgmode;
   if (BOSS.HP < minus) {
    minus = BOSS.HP;
   ...
   chaos = Math.floor(Math.random() * 9);
   if (skiller.type === "heavy" || skiller.type === "angry" || skiller.type === "vitalpoint" || skiller.type === "bodypress" || skiller.type === "physics") {
    score += Math.ceil(Math.ceil(minus * ((BOSS.B * rank[BR2].R) / Math.sqrt(PM[ID].A)) / Math.sqrt(skiller.power * 2) * 50) * randomscore[chaos].rate);
    //alert("スコア " + score + " minus " + minus + " power " + skiller.power + " rate " + BOSS.B * rank[BR2].R / PM[ID].A + " physics");
   } else {
    score += Math.ceil(Math.ceil(minus * ((BOSS.D * rank[DR2].R) / Math.sqrt(PM[ID].C)) / Math.sqrt(skiller.power * 2) * 50) * randomscore[chaos].rate);
...
function dmganimation2() {
 if (BOSS.HP > futurehp2) {
  ...
  //let MF = Math.ceil(BOSS.HP / BOSS.MHP * 100);
  //alert(hpbars + 1 + " " + MF);
  setTimeout(dp1,200);
 };
}
 ...
 if (nodmg === true) {
  ...
  if (skiller.Wattack === true) {
   ...
   if (AH === true || AH2 === true || AMH === true) {
    if (AH === true) {
     futurehp1 = PM[ID].HP + Math.ceil(minus / 2 / dmgmode) * dmgmode;
    };
    if (AH2 === true) {
     futurehp1 = PM[ID].HP + Math.floor(minus / 4 * 3 / dmgmode) * dmgmode;
    };
    if (AMH === true) {
     futurehp1 = PM[ID].HP + Math.floor(PM[ID].MHP / 6 / dmgmode) * dmgmode;
    };
...
function dp2() {
 if (nodmg === true) {
  ...
  if (Bskiller.Wattack === true) {
   ...
   if (AH === true || AH2 === true || AMH === true) {
    if (AH === true) {
     futurehp2 = BOSS.HP + Math.floor(minus / 2 / dmgmode) * dmgmode;
    };
    if (AH2 === true) {
     futurehp2 = BOSS.HP + Math.floor(minus / 4 * 3 / dmgmode) * dmgmode;
    };
    if (AMH === true) {
     futurehp2 = BOSS.HP + Math.floor(BOSS.MHP / 6 / dmgmode) * dmgmode;
    };
    if (futurehp2 > BOSS.MHP) {
     futurehp2 = BOSS.MHP;
    };
...
function testfor1() {
 ...
 if (PM[ID].HP <= 0) {
  if (recalled === false && recall === false) {
   alert(PN1 + "は倒れた…。");
  };
  dier1 = true;
  if (PM[ID].MAX === true && recalled === false && recall === false) {
   PM[ID].MAX = false;
   PK1.style.width = 200;
  ...
  if (((BOSS.otherF === "しろのいななき" && AR2 !== 12) || (BOSS.otherF === "くろのいななき" && CR2 !== 12)) && recalled === false) {
   if (recall === true) {
    recall = false;
    if (BOSS.otherF === "しろのいななき" && AR2 !== 12) {
     AR2 += 1;
     ARdp2.innerHTML = rank[AR2].dp;
     alert(PN2 + "のこうげきが上がった！");
    };
    if (BOSS.otherF === "くろのいななき" && CR2 !== 12) {
     CR2 += 1;
   ...
   if (dies === diept) {
    ...
    score = Math.floor(score * Srate);
    localStorage.setItem("task1",false);
    if (task2 <= score) {
     taskclear += 1;
     score = Math.ceil(score / 10 * 13);
     localStorage.setItem("task2",true);
    } else {
     localStorage.setItem("task2",false);
    };
    localStorage.setItem("task3",false);
    localStorage.setItem("taskclear",taskclear);
    localStorage.setItem("score",score);
    //alert("スコアは" + score + "点でした！");
    window.location.href = "result(event).html";
   } else {
    down = true;
...
function testfor2() {
 ...
 if (BOSS.HP <= 0) {
  ...
  score += Math.floor(40000 / turn) + 30000 + Math.floor(BOSS.MHP / turn * 200);
  score = Math.floor(score * Srate);
  localStorage.setItem("task1",true);
  taskclear += 1;
  if (PM[0].bodytype1 === task3 || PM[0].bodytype2 === task3 || PM[1].bodytype1 === task3 || PM[1].bodytype2 === task3 || PM[2].bodytype1 === task3 || PM[2].bodytype2 === task3 || PM[3].bodytype1 === task3 || PM[3].bodytype2 === task3 || PM[4].bodytype1 === task3 || PM[4].bodytype2 === task3 || PM[5].bodytype1 === task3 || PM[5].bodytype2 === task3) {
   localStorage.setItem("task3",false);
  } else {
      ...
      if (F2 === "ダルマモード" && BOSS.daruma === false && BOSS.HP <= BOSS.MHP / 2) {
       ...
       if ((F2 === "スワームチェンジ" && BOSS.perfect !== true && BOSS.HP <= BOSS.MHP / 2) || swarm === true) {
        if (swarm === true) {
         ...
         PK2.src = BOSS.img;
         BOSS.MHP = BOSS.MHP + Math.ceil(216 * HPrate);
         BOSS.S -= Math.ceil(10 * Rrate);
         futurehp2 = BOSS.HP + Math.ceil(216 * HPrate / dmgmode) * dmgmode;
         hpbars = Math.ceil(BOSS.HP / BOSS.MHP * 100) - 1;
