 ...
 if (turn > 2) {
  if (tries === 0) {
   ...
   if (tries === 4) {
    ...
    if (BOSS.name === "ディアンシー" && typer[1] <= typer[0] && typer[2] <= typer[0] && typer[3] <= typer[0] && (typer[0] === typer[1] || typer[0] === typer[2] || typer[0] === typer[3])) {
     typer[0] += 1;
    };
    //if (BOSS.name === "キュレム" && BOSS.skill1 === 33 && (typer[0] === typer[1] || typer[0] === typer[2] || typer[0] === typer[3])) {
    // typer[0] -= 1;
    //};
  ...
  if (BOSS.MAX === true) {
   ...
   MAXskill[BOSS.skill3].power = BOSS.power3;
   MAXskill[BOSS.skill4].power = BOSS.power4;
   select[0] = MAXskill[BOSS.skill1];
   select[1] = MAXskill[BOSS.skill2];
   select[2] = MAXskill[BOSS.skill3];
   select[3] = MAXskill[BOSS.skill4];
  } else {
   select[0] = skill[BOSS.skill1];
   select[1] = skill[BOSS.skill2];
   select[2] = skill[BOSS.skill3];
   select[3] = skill[BOSS.skill4];
  };
  chaos = Math.floor(Math.random() * 4);
  Bskiller = select[chaos];
  tries = 0;
  selected = 0;
  test1();
...
function PKCOMAX2() {
 let howfardown = (AR2 + BR2 + CR2 + DR2 + SR2) * -1 + 30;
 if (howfardown < 0) {
  howfardown = 0;
 };
 if (swap2 === true) {
 ...
 chaos = Math.random() * 100;
 if (skiller.name !== "ゴーストダイブ") {
  if ((chaos <= 5 * howfardown && (AR2 < 6 || BR2 < 6 || CR2 < 6 || DR2 < 6 || SR2 < 6 || KR2 < 6))) {
   if (AR2 < 6) {
    AR2 = 6;
   };
   if (BR2 < 6) {
   ...
   if (chaos <= 10 && F1 !== "null") {
    ...
    chaos = Math.random() * 100;
    if (chaos <= 10 && (AR1 > 6 || BR1 > 6 || CR1 > 6 || DR1 > 6 || SR1 > 6 || KR1 > 6)) {
     AR1 = 6;
     BR1 = 6;
     CR1 = 6;
     DR1 = 6;
